...
class Sweeper(object):
    ...
    def generate_config_for_idx(self, idx):
        ...
        Index is from 1 to # of combinations.
        '''
...
if __name__ == "__main__":
    for agent_config in os.listdir('./configs/toy/'):
        config_file = os.path.join('./configs/toy/', agent_config)
        if not os.path.isfile(config_file): continue
        sweeper = Sweeper(config_file)
        sweeper.print_config_dict(sweeper.config_dicts)
        #sweeper.print_config_dict(sweeper.generate_config_for_idx(2600))
