...
class GymPendulumConfigModule:
    ...
    TASK_HORIZON = 200 #1000, not sure why it's set to 1000 before
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 30 # how is it picked?
    INIT_VAR = 0.25 # 0.5 # 0.25
    ...
    @staticmethod
    def get_display_obs(obs):
        """ return a dictionary of tensors for tf summary """
        y = obs[:, 0] # cos(theta)
        x = obs[:, 1] # sin(theta)
        theta = tf.atan2(x, y) # here we use this order (x,y) to get the angle wrt the +y axis rather than +x axis
        thetadot = obs[:, 2]
        return {"theta": theta, "theta_dot": thetadot}
    ...
    def obs_cost_fn(obs):
        ...
        if isinstance(obs, np.ndarray):
            ...
            theta = tf.atan2(x, y) # here we use this order (x,y) to get the angle wrt the +y axis rather than +x axis
        thetadot = obs[:, 2]
        # theta is already in (-pi, pi) so no need to normalize
        cost = theta ** 2 + 0.1 * thetadot ** 2
        return cost

    @staticmethod
    def obs_cost_fn_bk(obs):
        """ @brief:
                see mbbl.env.gym_env.walker.py for reward details
