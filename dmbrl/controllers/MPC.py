...
from dmbrl.misc.optimizers import RandomOptimizer, CEMOptimizer, DecentCEMOptimizer
...
from dmbrl.misc.optimizers import POPLINAEOptimizer
from dmbrl.misc.optimizers import POPLINAESOptimizer
from dmbrl.misc.optimizers import POPLINPIOptimizer
...
class MPC(Controller):
    optimizers = {"CEM": CEMOptimizer, "Random": RandomOptimizer,
                  "DecentCEM": DecentCEMOptimizer,
                  'GBPRandom': GBPRandomOptimizer, 'GBPCEM': GBPCEMOptimizer,
                  'POPLIN-A': POPLINAOptimizer, 'POPLIN-P': POPLINPOptimizer, 
                  'POPLIN-PI': POPLINPIOptimizer, 'POPLIN-A-E': POPLINAEOptimizer,
                  'POPLIN-P-E': POPLINPEOptimizer,'POPLIN-AES': POPLINAESOptimizer, 
                  'MPC-PI': MPCPIOptimizer, 'MPC-PI-Sampler': MPCPISamplerOptimizer}
    ...
    def __init__(self, params):
        ...
        self.get_display_obs_fn = params["get_display_obs_fn"]
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, train_policy=True, episode=None):
        ...
        if self.optimizer.train_policy_network() and self.has_been_trained and train_policy:
            # train the policy network here
            self.optimizer.train(obs_trajs, acs_trajs, rews_trajs, episode=episode)

        self.has_been_trained = True

    def construct_tf_summary(self, logdir):
        ''' the policy net is either a list (for ensemble) or a single one '''
        if isinstance(self._policy_network, list):
            writer = None
            for i, pn in enumerate(self._policy_network):
                pn.construct_tf_summary(logdir, self.get_display_obs_fn, writer)
                if i == 0:
                    writer = pn.get_tf_writer() # pass the same writer in for i>=1
        elif self._policy_network is not None:
            self._policy_network.construct_tf_summary(logdir, self.get_display_obs_fn)

        # tf writer only available here
        self.set_optimizer_tf_writer(logdir)

    def get_tf_writer(self):
        if isinstance(self._policy_network, list):
            return self._policy_network[0].get_tf_writer()
        elif self._policy_network is not None:
            return self._policy_network.get_tf_writer()
        else:
            return self.optimizer.get_tf_writer()

    def set_optimizer_tf_writer(self, logdir):
        if self._params.opt_cfg.mode in ['CEM', "DecentCEM"]:
            self.optimizer.set_tf_writer(logdir)
        else:
            self.optimizer.set_tf_writer()
    ...
    def reset(self):
        ...
        """
        self.prev_sol = np.tile((self.ac_lb + self.ac_ub) / 2, [self.plan_hor])
        self.prev_sol = self.optimizer.reset_prev_sol(self.prev_sol)  # HACK for poplin_p
        ...
        if test_policy:
            if self.model.is_tf_model:
                ...
                soln, self.prev_sol = self.optimizer.obtain_test_solution(
                    self.prev_sol, self.init_var, self.per, self.dU, obs=obs, average=average, ctrl_type=self._params.cem_cfg.eval_ctrl_type
                )
            else:
                soln, self.prev_sol = self.optimizer.obtain_test_solution(
    ...
    def set_logdir(self, primary_logdir):
        """
            Args:
        ...
        """
        pass
        #self.optimizer.set_logdir(primary_logdir)

    def dump_logs(self, primary_logdir, iter_logdir, step=None):
        ...
        Arguments:
            ...
            step (int): the global steps to name the ckpt. Default: None

        Returns: None
        """
        self.model.save(iter_logdir if self.save_all_models else primary_logdir)
        # save policy network
        self.optimizer.save(os.path.join(primary_logdir, 'policynet_ckpt'), step)
    ...
    def _compile_POPLINA_cost(self, ac_noise_seqs, cem_type, tf_data_dict):
        ...
        if cem_type == 'POPLINA-INIT':
            ...
            def iteration(t, total_cost, cur_obs):
                ...
                return t + 1, total_cost + tf.pow(self.gamma, t) * delta_cost, \
                    self.obs_postproc2(next_obs), cur_acs
    ...
    def _compile_cost(self, ac_seqs, get_pred_trajs=False,
        ...
        init_costs = tf.zeros([nopt, self.npart])
        # ac_seq comes in as a population of seq of actions
        # MPC-PI ac_seq shape = popsize, sol_dim -> popsize, plan_hor, dU
        ac_seqs = tf.reshape(ac_seqs, [-1, self.plan_hor, self.dU])
        # -> plan_hor, popsize, 1, dU -> plan_hor, popsize, npart, dU 
