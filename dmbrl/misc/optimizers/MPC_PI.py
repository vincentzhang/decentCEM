...
class MPCPIOptimizer(Optimizer):
    ...
    def __init__(self, sol_dim, max_iters, popsize, num_elites=None, tf_session=None,
        ...
        with self.tf_sess.graph.as_default():
            with tf.variable_scope("bc_policy_network"):
                if self._params.cem_cfg.training_scheme in ['BC-KL']:
                    self._policy_network = BC_KL_policy.policy_network(
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, imaginary_replay_buffer=None, episode=None):
        """ For training of the policy net, we would need the following
            ...
            We do not really need to store the true action separately anymore.
        ...
        if imaginary_replay_buffer:
            imaginary_dataset = {
                key: [] for key in ['start_state', 'action_samples', 'target_pi', 'num_samples', 'returns', 'weights', 'target_mu', 'target_std', 'target_pdf']
        ...
        with self.tf_sess.graph.as_default():
            ...
            if self._cem_type == 'Gaussian':
                ...
                for t in range(self.plan_hor):
                    ...
                    policy_mean, policy_std = self._policy_network.forward_network(cur_obs) # shape (popsize, dU)
                    ...
                    proposed_act = policy_dist.sample() # shape (popsize, dU)
                    #if t == 0:
    ...
    def setup(self, cost_function, tf_compatible):
        ...
        self.pi_std = tf.sqrt( weighted_var )
        ...
        self.target_pi = self.pi / tf.reduce_sum(self.pi, axis=0) # this normalizes the pdf on each action dim such that it's a pmf over this discrete support
    ...
    def obtain_solution(self, init_mean, init_var, per, dU, obs=None):
        ...
        if self._params.cem_cfg.training_scheme == 'BC-KL':
    ...
    def obtain_test_solution(self, init_mean, init_var, per, dU, obs=None, average=False, ctrl_type='PolicyControl'):
        ...
        if average:
            ...
            if ctrl_type == 'PolicyControl':
                ...
                # no need for prev_sol since we don't do CEM anymore, just use the action form policy network
